<template>
  <AppShell :navs="navs" search-placeholder="搜索申请单...">
    <div v-if="d" class="flex flex-col gap-5">
      <!-- Breadcrumb（与 TaskDetailView 完全相同的结构） -->
      <div class="flex items-center gap-2 text-sm" style="color:var(--muted-foreground)">
        ...
        <span>/</span><span class="truncate font-medium" style="color:var(--foreground)">{{ d.applicationTitle || d.applicationNo || '申请单确认' }}</span>
      </div>

      <!-- Title row（与 TaskDetailView 完全相同的结构） -->
      ...
      <!-- 申请单信息卡片（与 TaskDetailView 的 content-card 结构完全一致：标题栏 + 主体） -->
      <article class="content-card" style="padding:calc(var(--spacing)*5)">
        <div class="flex items-center gap-2 mb-4">
          <FileText :size="18" style="color:var(--primary)" />
          <span class="text-xs font-semibold uppercase" style="letter-spacing:.08em;color:var(--muted-foreground)">申请单信息</span>
        </div>
      ...
      <!-- 双列对比：供应商信息 + 商品信息（与 TaskDetailView 的双列 ContractCard 布局一致） -->
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <article class="content-card" style="padding:calc(var(--spacing)*5)">
          <div class="flex items-center gap-2 mb-4">
            <Package :size="18" style="color:var(--primary)" />
            <span class="text-xs font-semibold uppercase" style="letter-spacing:.08em;color:var(--muted-foreground)">供应商信息</span>
          </div>
        ...
        <article class="content-card" style="padding:calc(var(--spacing)*5)">
          <div class="flex items-center gap-2 mb-4">
            ...
            <span class="text-xs font-semibold uppercase" style="letter-spacing:.08em;color:var(--muted-foreground)">商品信息</span>
          </div>
          <div class="flex flex-col gap-3">
      ...
      <!-- 备注信息卡片（与 TaskDetailView 的 ChatPanel 卡片结构一致：标题栏 border-bottom + 主体） -->
...
import { ChevronLeft, Check, FileText, Package, ShoppingCart, MessageSquare, ListTodo, ClipboardList } from 'lucide-vue-next'
...
// 与 TaskDetailView 完全相同的 navs 结构
const navs = [
